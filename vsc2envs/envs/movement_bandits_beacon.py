...
class MovementBanditsMoveToBeacon(gym.Env):
    metadata = {
    ...
    def __init__(self):
        ...
        self.observation_space = spaces.Box(-10000000, 10000000, shape=(3+self.numgoals,))
    ...
    def _step(self, action):
        # print('action ' , action)
        def translate_action_1d_to_2d(id):
            max = self.action_size*self.action_size
            assert(id >= 0 or id < max)
            yth_elem = math.floor(id / self.action_size)
            xth_elem = id - (yth_elem * self.action_size)
            each_sq_sz = self.screen_size/self.action_size
            half_length = each_sq_sz / 2
            ret = [xth_elem * each_sq_sz + half_length,
                    yth_elem * each_sq_sz + half_length]
            return ret

        [x,y] = translate_action_1d_to_2d(action)
        self.x_clicked = x
        self.y_clicked = y
